...
SentryArmorDetector::SentryArmorDetector()
{
    initializeVariate();
}
SentryArmorDetector::~SentryArmorDetector()
...
void SentryArmorDetector::initializeVariate()
{
    armordata.is_get = 0;
    armordata.shoot = 0;
    rectLists.clear();
...
bool SentryArmorDetector::judgeRectangleIsRight(const RotatedRect &rRect, const float &rRectArea)
{
 ...
 * @Throw: 
 */
void SentryArmorDetector::colorThres(cv::Mat &opImage)
{
    Mat gray_img;
    vector<Mat> splited;
...
void SentryArmorDetector::getLights()
{
    RotatedRect rGMax;
    rGMax = rectLists[0];
    int cellMaxSize;
...
// 得到所有可能的装甲板并分为大小两类
void SentryArmorDetector::getArmors(cv::Mat &ipImg)
{
    Point2f LC1, LC2;
...
void SentryArmorDetector::getArmor(cv::Mat &ipImg)
{
...
bool SentryArmorDetector::angleCmp(cv::RotatedRect &angle1, cv::RotatedRect &angle2) //暂无使用，暂保留
{
    return angle1.angle < angle2.angle;
}

bool SentryArmorDetector::sizeCmp(cv::RotatedRect &size1, cv::RotatedRect &size2) //暂无使用，暂保留
{
    return size1.size.width * size1.size.height > size2.size.width * size2.size.height;
}

bool SentryArmorDetector::rxCmp(cv::RotatedRect &rx1, cv::RotatedRect &rx2)
{
    return rx1.center.x < rx2.center.x;
}

void SentryArmorDetector::xCmp(cv::RotatedRect &x1, cv::RotatedRect &x2) //暂无使用，暂保留
{
...
int SentryArmorDetector::whiteSums(cv::Mat &src)
{
...
void SentryArmorDetector::drawRRects(cv::Mat &ipImg, std::vector<cv::RotatedRect> RectVectors)
{
