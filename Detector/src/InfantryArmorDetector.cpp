    ...
    for (int i = 0; i < num; i++)
    {
        for (int j = i + 1; j < num; j++)
        {
            ...
            if (mode == 3)
            {
                ...
                //连线长
                //两个矩形中心的连线长度
                if (d >= longi * 4.7 || d >= longj * 4.7 || d < 1.5 * longi || d < 1.5 * longi)
                    matchrank[i][j] -= 10000;
                else if (d <= longi * 4.7 && d <= longj * 4.7 && d > 1.5 * longi && d > 1.5 * longi)
                    matchrank[i][j] += 100;
                if (d >= longi * 3 || d >= longj * 3)
                    dis[i][j] = 0; //=1为小装甲
                else
                    dis[i][j] = 1;
            ...
            else if (mode == 1)
            {
                ...
                //中心连线长筛选
                if (d >= longi * 7 || d >= longj * 7 || d < 1.5 * longi || d < 1.5 * longj)
                    matchrank[i][j] -= 10000;
                else if (d >= longi * 2.7 || d >= longj * 2.7 || d < 1.5 * longi || d < 1.5 * longi)
                    matchrank[i][j] -= 10000;
                else if (d <= longi * 2.7 && d <= longj * 2.7 && d > 1.5 * longi && d > 1.5 * longi)
                    matchrank[i][j] += 100;
                cout << "i j d:" << i << " " << j << " " << d << endl;
            ...
            else
            {
                ...
                if (d >= longi * 7 || d >= longj * 7 || d < 1.5 * longi || d < 1.5 * longj)
                    matchrank[i][j] -= 10000;
                else if (d >= longi * 5 || d >= longj * 5 || d < 3 * longi || d < 3 * longi)
                    matchrank[i][j] -= 10000;
                else if (d <= longi * 5 && d <= longj * 5 && d > 3 * longi && d > 3 * longi)
                    matchrank[i][j] += 100;
                cout << "i j d:" << i << " " << j << " " << d << endl;
...
void InfantryArmorDetector::getTarget()
{
    ...
    if (mode == 2)
        ...
        roif = 20.0;
    if (target.rect[0].x - roif < 0)
        x = 0;
    else
        x = target.rect[0].x - roif;
    if (target.rect[0].y - roif < 0)
        y = 0;
    else
        y = target.rect[0].y - roif;
    roi.lefttop = Point2f(x, y);
    int h, w;
    w = target.rect[2].x - target.rect[0].x + 2 * roif;
    h = target.rect[3].y - target.rect[0].y + 2 * roif;
    //cout<<"w h:"<<w<<" "<<h<<endl;
    if (roi.lefttop.x + w > src.cols)
        roi.rwidth = src.cols - roi.lefttop.x;
    else
        roi.rwidth = w;
    if (roi.lefttop.y + h > src.rows)
        roi.rheight = src.rows - roi.lefttop.y;
    else
        roi.rheight = h;
    roiimg = src(Rect(roi.lefttop.x, roi.lefttop.y, roi.rwidth, roi.rheight));
}
    ...
    */
    ...
    if (mode == 1 || mode == 3)
    {
        if (armor_data.is_big)
        {
            ...
            newy = m10 * 113 + m11 * 27.5 + m12 * 0 + _t_matrix.at<double>(1, 0) - 60 - drop * 1000;
            ; //减往上-newz*newz/20000 -0.026x+88
        }
        else
        {
...
{
    if ((box[local_num_i].size.width > box[local_num_i].size.height && box[local_num_i].angle > -70) || (box[local_num_i].size.width < box[local_num_i].size.height && box[local_num_i].angle < -20))
        matchrank[local_num_i][local_num_j] -= 10000;
    if ((box[local_num_j].size.width > box[local_num_j].size.height && box[local_num_j].angle > -70) || (box[local_num_j].size.width < box[local_num_j].size.height && box[local_num_j].angle < -20))
        matchrank[local_num_i][local_num_j] -= 10000;
}
...
void InfantryArmorDetector::aspectRadioFilter(vector<RotatedRect> &box)
{
    ...
    //长宽比判断筛选
    if ((longi / shorti) <= 1.3 || (longj / shortj) <= 1.3)
        matchrank[local_num_i][local_num_j] -= 10000;
    else if ((longi / shorti) >= 9.5 || (longj / shortj) >= 9.5)
        matchrank[local_num_i][local_num_j] -= 10000;
    else if ((longi / shorti) >= 4 && (longi / shorti) <= 7 && (longj / shortj) >= 4 && (longj / shortj) <= 7)
        matchrank[local_num_i][local_num_j] += 100;
    else if ((longi / shorti) >= 3.5 && (longi / shorti) <= 7.5 && (longj / shortj) >= 3.5 && (longj / shortj) <= 7.5)
        matchrank[local_num_i][local_num_j] += 80;
}
void InfantryArmorDetector::relativePositionFilter(vector<RotatedRect> &box, ModeParam mode)
{
    if ((box[local_num_i].center.y - box[local_num_j].center.y) > mode.relative_position_radio * longi || (box[local_num_i].center.y - box[local_num_j].center.y) > mode.relative_position_radio * longj)
        matchrank[local_num_i][local_num_j] -= 10000;
    else if (abs(box[local_num_i].center.y - box[local_num_j].center.y) < 3)
        matchrank[local_num_i][local_num_j] += 100;
    else if (abs(box[local_num_i].center.y - box[local_num_j].center.y) < 5)
        matchrank[local_num_i][local_num_j] += 50;
    else if (abs(box[local_num_i].center.y - box[local_num_j].center.y) < 8)
        matchrank[local_num_i][local_num_j] += 10;
}
void InfantryArmorDetector::relativeTiltAngleFilter(ModeParam mode)
{
    if (abs(anglei - anglej) <= mode.relative_angle[0][0] || abs(anglei - anglej) >= mode.relative_angle[0][1])
        matchrank[local_num_i][local_num_j] += 200;
    else if (abs(anglei - anglej) <= mode.relative_angle[1][0] || abs(anglei - anglej) >= mode.relative_angle[1][1])
        matchrank[local_num_i][local_num_j] += 100;
    else if (abs(anglei - anglej) <= mode.relative_angle[2][0] || abs(anglei - anglej) >= mode.relative_angle[2][1])
        matchrank[local_num_i][local_num_j] += 50;
    else if (abs(anglei - anglej) <= mode.relative_angle[3][0] || abs(anglei - anglej) >= mode.relative_angle[3][1])
        matchrank[local_num_i][local_num_j] += 5;
    else
        matchrank[local_num_i][local_num_j] -= 10000;
}
void InfantryArmorDetector::areaRadioFilter(ModeParam mode)
{
    if (areai < mode.relative_area[0] || areaj < mode.relative_area[0])
        matchrank[local_num_i][local_num_j] -= 20000;
    else if (areai / areaj >= mode.relative_area[1])
        matchrank[local_num_i][local_num_j] -= 10000;
    else if (areai / areaj >= mode.relative_area[2])
        matchrank[local_num_i][local_num_j] -= 100;
    else if (areai / areaj > mode.relative_area[3])
        matchrank[local_num_i][local_num_j] += 10;
    else if (areai / areaj > mode.relative_area[4])
        matchrank[local_num_i][local_num_j] += 50;
    else
        matchrank[local_num_i][local_num_j] += 100;
}
...
{
    if (d >= longi * mode.center_distance[0][0] || d >= longj * mode.center_distance[0][0] || d < longi * mode.center_distance[0][1] || d < longj * mode.center_distance[0][1])
        matchrank[local_num_i][local_num_j] -= 10000;
    else if (d >= longi * mode.center_distance[1][0] || d >= longj * mode.center_distance[1][0] || d < mode.center_distance[1][1] * longi || d < mode.center_distance[1][1] * longj)
        matchrank[local_num_i][local_num_j] -= 10000;
    else if (d <= longi * mode.center_distance[1][0] && d <= longj * mode.center_distance[1][0] && d > mode.center_distance[1][1] * longi && d > mode.center_distance[1][1] * longj)
        matchrank[local_num_i][local_num_j] += 100;
...
void InfantryArmorDetector::pickBest()
{
    ...
    if (maxpoint < matchrank[local_num_i][local_num_j])
    {
        ...
        if (mode == 3)
        {
            if (dis[local_num_i][local_num_j] == 1)
                armor_data.is_big = false;
            else
                armor_data.is_big = true;
        }
...
void InfantryArmorDetector::lostArmor()
{
    ...
    if (roi.lefttop.x - 10 <= 0)
        ...
        roi.lefttop.x -= 10;
    if (roi.lefttop.y - 10 <= 0)
        roi.lefttop.y = 0;
    else
        roi.lefttop.y -= 10;
    if (roi.lefttop.x + roi.rwidth + 20 > src.cols)
        roi.rwidth = src.cols - roi.lefttop.x;
    else
        roi.rwidth = roi.rwidth + 20;
    if (roi.lefttop.y + roi.rheight + 20 > src.rows)
        roi.rheight = src.rows - roi.lefttop.y;
    else
        roi.rheight = roi.rheight + 20;
    roiimg = src(Rect(roi.lefttop.x, roi.lefttop.y, roi.rwidth, roi.rheight));
}
